...
def generate_xml(env):
    ...
    include_paths=env['CPPPATH'],keep_xml=True, flags=flags, castxml_epic_version=1)
            ...
            if clAss.name != "" and clAss.class_type == 'class' or ( not clAss.name.startswith('_') and clAss.class_type == 'struct') or clAss.class_type == 'union':
                ...
                for member in clAss.member_functions(allow_empty=True):
                    if member.access_type == 'public' and member.name != "":
                        ...
                        typ = str(member.return_type).replace('::','').replace('godot','').replace('const','')
                        returnType = ET.SubElement(method,'return',type=typ)
                        indx = 0
                        for args in member.arguments:
                            if str(args.decl_type) in member.decl_string:
                                typeText = str(args.decl_type).replace('::','').replace('godot','').replace('const','')
                ...
                for var in clAss.variables(allow_empty=True):
                    ...
                    if var.access_type == 'public':
                        typeText = str(var.decl_type).replace('::','').replace('godot','')
                    if '[' in typeText:
